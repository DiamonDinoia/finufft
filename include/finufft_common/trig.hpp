...
template<int TolDigits, typename T> struct approx_coefficients {
  static constexpr std::size_t nterms() {
    // if constexpr: MSVC's C4127 (constant conditional) fires on the plain
    // if-chain under /W4 /WX, and warnings are errors in CI
    if constexpr (TolDigits <= 4) {
      return 2;
    } else if constexpr (TolDigits <= 6) {
      return 3;
    } else if constexpr (TolDigits <= 8) {
      return 4;
    } else if constexpr (TolDigits <= 12) {
      return 5;
    } else {
      return 6;
    }
