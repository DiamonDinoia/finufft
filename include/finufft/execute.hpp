  ...
  if (type != 3) { // --------------------- TYPE 1,2 EXEC ------------------
    ...
    if (opts.debug > 1)
      printf("[%s] scratch: %s (%.3g MB)\n", "execute",
             scratch_provided ? "caller-provided aligned buffer"
                              : "thread-local reclaimable buffer",
             fwBatchBytes / 1e6);
    ...
    if (!adjoint) { // we can combine CpBatch and fwBatch_inner!
      buf1.resize(std::max(m.nj * batchSize, m.t3cache.innerT2plan->nf() *
                                                 m.t3cache.innerT2plan->batchSize));
    ...
    if (opts.debug > 1) {
      const char *layout = !adjoint
                               ? "CpBatch shares storage with inner fwBatch"
...
#pragma omp parallel for num_threads(opts.nthreads) // or batchSize?
       ...
       still the same size, as Andrea explained; just wastes a few flops) */
        m.t3cache.innerT2plan->execute_internal(
            fkb, fwBatch, adjoint, thisBatchSize, fwBatch_inner,
...
#pragma omp parallel for num_threads(opts.nthreads)
        for (BIGINT k = 0; k < m.nk; ++k)
          for (int i = 0; i < thisBatchSize; i++)
            fkb[i * m.nk + k] *= m.t3cache.deconv[k];
        t_deconv += timer.elapsedsec();
      } else { // adjoint mode
...
#pragma omp parallel for num_threads(opts.nthreads)
        ...
        timer.restart();
        m.t3cache.innerT2plan->execute_internal(
            CpBatch, fwBatch, adjoint, thisBatchSize, fwBatch_inner,
            m.t3cache.innerT2plan->nf() * m.t3cache.innerT2plan->batchSize);
        t_inner += timer.elapsedsec();
