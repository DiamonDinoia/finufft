...
#pragma omp parallel for num_threads(opts.nthreads) schedule(static)
      ...
      if (m.t3P.D[0] != 0.0 || m.t3P.D[1] != 0.0 || m.t3P.D[2] != 0.0) {
        ...
        if (opts.debug > 1) {
          if (nbatches != 0)
            printf("[%s t3] source prephase via xsimd sincos batches + scalar polar "
                   "tails (W=%zu)\n",
...
#pragma omp for schedule(static)
          for (BIGINT jb = 0; jb < nbatches; ++jb) {
            ...
            auto [sn, cs] = finufft::common::math::sincos(isign_batch * phase);
            cs.store_unaligned(re_buf.data());
            sn.store_unaligned(im_buf.data());
            for (std::size_t lane = 0; lane < batch_width; ++lane)
              tc.prephase[j + static_cast<BIGINT>(lane)] = TC(re_buf[lane], im_buf[lane]);
          }
#pragma omp for schedule(static)
...
#pragma omp parallel for num_threads(opts.nthreads) schedule(static)
    ...
    if (tgt_hit && src_hit) {
      ...
      if (do_phase) {
        ...
        if (opts.debug > 1) {
          if (nbatches != 0)
            printf("[%s t3] deconv phasing via xsimd sincos batches + scalar polar tails "
                   "(W=%zu)\n",
                   __func__, batch_width);
          else
...
#pragma omp for schedule(static)
          for (BIGINT kb = 0; kb < nbatches; ++kb) {
            ...
            for (int idim = 0; idim < dim; ++idim) {
              ...
              phase = xsimd::fma(batch(m.t3P.C[idim]), stu - batch(m.t3P.D[idim]), phase);
            }
            const auto inv = batch::load_unaligned(tc.invPhiHat.data() + k);
            auto [sn, cs]  = finufft::common::math::sincos(isign_batch * phase);
            (inv * cs).store_unaligned(re_buf.data());
            (inv * sn).store_unaligned(im_buf.data());
            for (std::size_t lane = 0; lane < batch_width; ++lane)
