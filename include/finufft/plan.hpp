...
private:
  ...
  struct type3params {
    // zero-init: entries past dim go into the type-3 cache keys
    std::array<TF, 3> X{}, C{}, D{}, h{}, gam{}; // X=halfwid C=center D=freqcen
                                                 // h,gam=rescale
  };
  ...
  struct M {
    ...
    // Cache for type-3 setpts. Owns all computed state that can be reused across
    // repeated setpts calls. Target-dependent and source-dependent work are
    // tracked independently so that changing only sources (or only targets)
    // skips the unaffected half.
    struct Type3Cache {
