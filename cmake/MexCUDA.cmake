...
function(_mexcuda_locate_exe OUT_VAR)
    ...
    if(MEXCUDA_EXECUTABLE AND EXISTS "${MEXCUDA_EXECUTABLE}")
        message(STATUS "[mexcuda] Using MEXCUDA_EXECUTABLE override: ${MEXCUDA_EXECUTABLE}")
        set(${OUT_VAR} "${MEXCUDA_EXECUTABLE}" PARENT_SCOPE)
        return()
    endif()
    ...
    if(UNIX)
        # 1) Try PATH via bash (type/command/which)
        set(_bash_probe
            "set -euo pipefail; (type -P mexcuda || command -v mexcuda || which mexcuda) 2>/dev/null || true"
        )
        execute_process(
            COMMAND /usr/bin/env bash -lc "${_bash_probe}"
            OUTPUT_VARIABLE _BASH_OUT
            RESULT_VARIABLE _BASH_RES
            OUTPUT_STRIP_TRAILING_WHITESPACE
            ERROR_QUIET
        )
        if(_BASH_OUT AND EXISTS "${_BASH_OUT}")
            set(_FOUND "${_BASH_OUT}")
        endif()

        # 2) If not found, try a slow exhaustive find under Matlab_ROOT_DIR
        if(NOT _FOUND AND Matlab_ROOT_DIR)
            # Follow symlinks and look for an executable named exactly 'mexcuda'
            set(_bash_find
                "set -euo pipefail; f=$(find -L \"${Matlab_ROOT_DIR}\" -type f -name mexcuda -perm -111 -print -quit 2>/dev/null || true); if [ -n \"$f\" ]; then echo \"$f\"; fi"
            )
            execute_process(
                COMMAND /usr/bin/env bash -lc "${_bash_find}"
                OUTPUT_VARIABLE _BASH_FIND_OUT
                RESULT_VARIABLE _BASH_FIND_RES
                OUTPUT_STRIP_TRAILING_WHITESPACE
                ERROR_QUIET
            )
            if(_BASH_FIND_OUT AND EXISTS "${_BASH_FIND_OUT}")
                set(_FOUND "${_BASH_FIND_OUT}")
            endif()
        endif()
    endif()

    if(WIN32 AND NOT _FOUND)
        # 1) Try PATH via cmd's 'where'
        execute_process(
            COMMAND cmd /C where mexcuda
            OUTPUT_VARIABLE _WHERE_OUT
            RESULT_VARIABLE _WHERE_RES
            OUTPUT_STRIP_TRAILING_WHITESPACE
            ERROR_QUIET
        )
        if(_WHERE_RES EQUAL 0 AND _WHERE_OUT AND EXISTS "${_WHERE_OUT}")
            set(_FOUND "${_WHERE_OUT}")
        endif()

        # 2) Slow exhaustive search under Matlab_ROOT_DIR via PowerShell
        if(NOT _FOUND AND Matlab_ROOT_DIR)
            set(_ps_cmd
                "$p='${Matlab_ROOT_DIR}'; if (-not [string]::IsNullOrWhiteSpace($p)) { $hit = Get-ChildItem -LiteralPath $p -Recurse -File -ErrorAction SilentlyContinue | Where-Object { $_.Name -match '^mexcuda(\.bat|\.exe)?$' } | Select-Object -First 1 -ExpandProperty FullName; if ($hit) { Write-Output $hit } else { exit 1 } } else { exit 1 }"
            )
            execute_process(
                COMMAND powershell -NoProfile -ExecutionPolicy Bypass -Command ${_ps_cmd}
                OUTPUT_VARIABLE _PS_OUT
                RESULT_VARIABLE _PS_RES
                OUTPUT_STRIP_TRAILING_WHITESPACE
                ERROR_QUIET
            )
            if(_PS_RES EQUAL 0 AND _PS_OUT AND EXISTS "${_PS_OUT}")
                set(_FOUND "${_PS_OUT}")
            endif()
        endif()
    endif()

    # 3) Final fallback: CMake-side recursive glob (may still miss on some filesystems)
    if(NOT _FOUND AND Matlab_ROOT_DIR)
        file(
        ...
        if(_CANDIDATES)
            # Prefer architecture subdir if present
            set(_BEST "")
    ...
    if(NOT _FOUND)
        message(
            FATAL_ERROR
            "[mexcuda] Could not find 'mexcuda'. Consider setting MEXCUDA_EXECUTABLE or Matlab_ROOT_DIR."
        )
    endif()

    message(STATUS "[mexcuda] Using mexcuda executable: ${_FOUND}")
