...
int main() {
  ...
  {
    // tol must be representable in FLT (1e-12/1e-6 are both rejected for float)
    static constexpr FLT cache_tol = std::is_same_v<FLT, float> ? FLT(1e-4) : FLT(1e-12);
    const auto run_type3 = [](const std::vector<FLT> &x, const std::vector<FLT> &s,
      ...
      BIGINT nmodes[1] = {0};
      int ier           = FINUFFT_MAKEPLAN(3, 1, nmodes, +1, 1, cache_tol, &plan, &opts);
      if (ier != 0) {
