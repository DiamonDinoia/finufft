...
template<typename T>
T relerrtwonorm(const std::complex<T> *a, const std::complex<T> *b, const std::size_t n,
...
auto almost_equal(V *d_vec, T *cpu, const std::size_t size,
  ...
  const auto error = relerrtwonorm(h_vec.data(), cpu, size, print);
...
int main() {
  ...
  const auto test_type3 = [iflag, tol, ntransf, dim, cpu_planer, deconv_tol, M, N,
    ...
    cudaDeviceSynchronize();
    assert(plan->type3_params.X1 == cpu_plan->t3P.X[0]);
    if (dim > 1) assert(plan->type3_params.X2 == cpu_plan->t3P.X[1]);
    if (dim > 2) assert(plan->type3_params.X3 == cpu_plan->t3P.X[2]);
    assert(plan->type3_params.C1 == cpu_plan->t3P.C[0]);
    if (dim > 1) assert(plan->type3_params.C2 == cpu_plan->t3P.C[1]);
    if (dim > 2) assert(plan->type3_params.C3 == cpu_plan->t3P.C[2]);
    assert(plan->type3_params.D1 == cpu_plan->t3P.D[0]);
    if (dim > 1) assert(plan->type3_params.D2 == cpu_plan->t3P.D[1]);
    if (dim > 2) assert(plan->type3_params.D3 == cpu_plan->t3P.D[2]);
    assert(plan->type3_params.gam1 == cpu_plan->t3P.gam[0]);
    if (dim > 1) assert(plan->type3_params.gam2 == cpu_plan->t3P.gam[1]);
    if (dim > 2) assert(plan->type3_params.gam3 == cpu_plan->t3P.gam[2]);
    ...
    if (dim > 1) assert(equal(plan->ky, cpu_plan->XYZ[1], M));
    if (dim > 2) assert(equal(plan->kz, cpu_plan->XYZ[2], M));
    ...
    if (dim > 2) assert(plan->t2_plan->nf3 == cpu_plan->innerT2plan->nfdim[2]);
    ...
    assert(plan->t2_plan->ms == cpu_plan->innerT2plan->mstu[0]);
    assert(plan->t2_plan->mt == cpu_plan->innerT2plan->mstu[1]);
    assert(plan->t2_plan->mu == cpu_plan->innerT2plan->mstu[2]);
    ...
    T *phiHat[]    = {cpu_plan->innerT2plan->phiHat[0].data(),
                      cpu_plan->innerT2plan->phiHat[1].data(),
                      cpu_plan->innerT2plan->phiHat[2].data()};
    ...
    if (ntransf == 1) { // cpu and gpu handle batching differently
      std::cout << "CpBatch : ";
      assert(almost_equal(plan->CpBatch, cpu_plan->CpBatch.data(), M, tol, false));
      std::cout << "fw : ";
      assert(almost_equal(plan->fw, cpu_plan->fwBatch.data(), plan->nf, tol * 10, false));
      std::cout << "t2_plan->fw : ";
