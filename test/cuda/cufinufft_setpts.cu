...
int main() {
  ...
  fin_opts.debug     = 2;
  fin_opts.upsampfac = upsampfact;
  const int iflag    = 1;
  const float tol    = 1e-8;
  const int ntransf  = 1;
  const int dim      = 3;
  int n_modes[3]     = {2, 5, 10};
  ...
  for (int i = 0; i < N; i++) {
    s[i] = (n_modes[0] / 2) * randm11();
    t[i] = (n_modes[1] / 2) * randm11();
    u[i] = (n_modes[2] / 2) * randm11();
  }
  ...
  auto test_type3 = [iflag,
                     ...
                     &opts](auto plan) {
    // plan is a pointer to a type that contains real_t
    using T             = typename std::remove_pointer_t<decltype(plan)>::real_t;
    const int type      = 3;
    const auto cpu_plan = cpu_planer(type);
    assert(cufinufft_makeplan_impl<T>(type, dim, (int *)n_modes, iflag, ntransf, T(tol),
                                      &plan, &opts) == 0);
    ...
    assert(plan->type3_params.X2 == cpu_plan->t3P.X[1]);
    assert(plan->type3_params.X3 == cpu_plan->t3P.X[2]);
    assert(plan->type3_params.C1 == cpu_plan->t3P.C[0]);
    assert(plan->type3_params.C2 == cpu_plan->t3P.C[1]);
    assert(plan->type3_params.C3 == cpu_plan->t3P.C[2]);
    assert(plan->type3_params.D1 == cpu_plan->t3P.D[0]);
    assert(plan->type3_params.D2 == cpu_plan->t3P.D[1]);
    assert(plan->type3_params.D3 == cpu_plan->t3P.D[2]);
    assert(plan->type3_params.gam1 == cpu_plan->t3P.gam[0]);
    assert(plan->type3_params.gam2 == cpu_plan->t3P.gam[1]);
    assert(plan->type3_params.gam3 == cpu_plan->t3P.gam[2]);
    ...
    assert(equal(plan->ky, cpu_plan->XYZ[1], M));
    assert(equal(plan->kz, cpu_plan->XYZ[2], M));
    assert(equal(plan->d_Sp, cpu_plan->STUp[0].data(), N));
    assert(equal(plan->d_Tp, cpu_plan->STUp[1].data(), N));
    assert(equal(plan->d_Up, cpu_plan->STUp[2].data(), N));
    // NOTE:seems with infnorm we are getting at most 11 digits of precision
    assert(almost_equal(plan->prephase, cpu_plan->prephase.data(), M, tol * T(1e-2)));
    assert(almost_equal(plan->deconv, cpu_plan->deconv.data(), N, tol * T(1e-2)));
    assert(cufinufft_destroy_impl<T>(plan) == 0);
    assert(finufft_destroy(cpu_plan) == 0);
    plan = nullptr;
    cudaDeviceSynchronize();
