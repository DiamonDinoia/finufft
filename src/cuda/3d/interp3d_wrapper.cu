...
struct Interp3DDispatcher {
  ...
  int operator()(int nf1, int nf2, int nf3, int M, cufinufft_plan_t<T> *d_plan,
    ...
    case 3:
      return cuinterp3d_output_driven<T, ns>(nf1, nf2, nf3, M, d_plan, blksize);
    default:
      std::cerr << "[cuinterp3d] error: incorrect method, should be 1, 2 or 3\n";
...
int cuinterp3d_output_driven(int nf1, int nf2, int nf3, int M, cufinufft_plan_t<T> *d_plan,
  ...
  if (d_plan->opts.gpu_kerevalmeth) {
    if (const auto finufft_err =
            cufinufft_set_shared_memory(interp_3d_output_driven<T, 1, ns>, 3, *d_plan)) {
      return FINUFFT_ERR_INSUFFICIENT_SHMEM;
    }
    cudaFuncSetSharedMemConfig(interp_3d_output_driven<T, 1, ns>,
                               cudaSharedMemBankSizeEightByte);
    RETURN_IF_CUDA_ERROR
    for (int t = 0; t < blksize; t++) {
      interp_3d_output_driven<T, 1, ns>
    ...
    if (const auto finufft_err =
            cufinufft_set_shared_memory(interp_3d_output_driven<T, 0, ns>, 3, *d_plan)) {
      return FINUFFT_ERR_INSUFFICIENT_SHMEM;
    }
    cudaFuncSetSharedMemConfig(interp_3d_output_driven<T, 0, ns>,
                               cudaSharedMemBankSizeEightByte);
    RETURN_IF_CUDA_ERROR
    for (int t = 0; t < blksize; t++) {
      interp_3d_output_driven<T, 0, ns>
          <<<totalnumsubprob, std::min(256, std::max(ns * ns * ns, np)),
             sharedplanorysize, stream>>>(
              d_kx, d_ky, d_kz, d_c + t * M, d_fw + t * nf1 * nf2 * nf3, M, nf1, nf2, nf3,
              es_c, es_beta, sigma, d_binstartpts, d_binsize, bin_size_x, bin_size_y,
