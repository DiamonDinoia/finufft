...
struct Interp2DDispatcher {
  ...
  int operator()(int nf1, int nf2, int M, cufinufft_plan_t<T> *d_plan,
    ...
    case 3:
      return cuinterp2d_output_driven<T, ns>(nf1, nf2, M, d_plan, blksize);
    default:
      std::cerr << "[cuinterp2d] error: incorrect method, should be 1, 2 or 3\n";
...
int cuinterp2d_output_driven(int nf1, int nf2, int M, cufinufft_plan_t<T> *d_plan,
  ...
  if (d_plan->opts.gpu_kerevalmeth) {
    if (const auto finufft_err =
            cufinufft_set_shared_memory(interp_2d_output_driven<T, 1, ns>, 2, *d_plan)) {
      return FINUFFT_ERR_INSUFFICIENT_SHMEM;
    }
    for (int t = 0; t < blksize; t++) {
      interp_2d_output_driven<T, 1, ns>
    ...
    if (const auto finufft_err =
            cufinufft_set_shared_memory(interp_2d_output_driven<T, 0, ns>, 2, *d_plan)) {
      return FINUFFT_ERR_INSUFFICIENT_SHMEM;
    }
    for (int t = 0; t < blksize; t++) {
      interp_2d_output_driven<T, 0, ns>
          <<<totalnumsubprob, 256, sharedplanorysize, stream>>>(
              d_kx, d_ky, d_c + t * M, d_fw + t * nf1 * nf2, M, nf1, nf2, es_c, es_beta,
              sigma, d_binstartpts, d_binsize, bin_size_x, bin_size_y, d_subprob_to_bin,
              d_subprobstartpts, d_numsubprob, maxsubprobsize, numbins[0], numbins[1],
