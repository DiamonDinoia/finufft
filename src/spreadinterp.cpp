  ...
  for (size_t i = 0; i < sizeof...(elems); ++i) {
    ...
    if constexpr (kerevalmeth == 1) {
      if (opts.upsampfac == 2.0) {
        eval_kernel_vec_Horner<T, ns, 200, simd_type>(ker + (i * ::finufft::common::MAX_NSPREAD), inputs[i],
                                                      opts);
      }
      if (opts.upsampfac == 1.25) {
        eval_kernel_vec_Horner<T, ns, 125, simd_type>(ker + (i * ::finufft::common::MAX_NSPREAD), inputs[i],
                                                      opts);
      }
    }
    if constexpr (kerevalmeth == 0) {
      alignas(simd_type::arch_type::alignment()) std::array<T, ::finufft::common::MAX_NSPREAD> kernel_args{};
      set_kernel_args<T, ns>(kernel_args.data(), inputs[i]);
      evaluate_kernel_vector<T, ns>(ker + (i * ::finufft::common::MAX_NSPREAD), kernel_args.data(), opts);
    }
  }
  return ker;
}
...
static void spread_subproblem_1d_dispatch(
  ...
  /* this is a dispatch function that will call the correct kernel based on the ns
   ...
   if (ns == MAX_NSPREAD) {
     if (opts.kerevalmeth) {
       return spread_subproblem_1d_kernel<::finufft::common::MAX_NSPREAD, true>(off1, size1, du, M, kx, dd,
       opts);
    } else {
       return spread_subproblem_1d_kernel<::finufft::common::MAX_NSPREAD, false>(off1, size1, du, M, kx, dd,
       opts);
    }
   if (ns == MAX_NSPREAD-1) {
     if (opts.kerevalmeth) {
       return spread_subproblem_1d_kernel<::finufft::common::MAX_NSPREAD-1, true>(off1, size1, du, M, kx, dd,
       opts);
    } else {
       return spread_subproblem_1d_kernel<::finufft::common::MAX_NSPREAD-1, false>(off1, size1, du, M, kx,
       dd, opts);
    }
  }
  ...
  ...
  */
  static_assert(::finufft::common::MIN_NSPREAD <= NS && NS <= ::finufft::common::MAX_NSPREAD,
                "NS must be in the range (MIN_NSPREAD, MAX_NSPREAD)");
  if constexpr (NS == ::finufft::common::MIN_NSPREAD) { // Base case
    if (opts.kerevalmeth)
      return spread_subproblem_1d_kernel<T, ::finufft::common::MIN_NSPREAD, true>(off1, size1, du, M, kx, dd,
                                                               opts);
    else {
      return spread_subproblem_1d_kernel<T, ::finufft::common::MIN_NSPREAD, false>(off1, size1, du, M, kx,
                                                                dd, opts);
    }
  } else {
    if (opts.nspread == NS) {
...
*/
{
  spread_subproblem_1d_dispatch<T, ::finufft::common::MAX_NSPREAD>(off1, size1, du, M, kx, dd, opts);
}
...
void spread_subproblem_2d_dispatch(
  ...
  if constexpr (NS == ::finufft::common::MIN_NSPREAD) { // Base case
    if (opts.kerevalmeth)
      return spread_subproblem_2d_kernel<T, ::finufft::common::MIN_NSPREAD, true>(off1, off2, size1, size2,
                                                               du, M, kx, ky, dd, opts);
    else {
      return spread_subproblem_2d_kernel<T, ::finufft::common::MIN_NSPREAD, false>(off1, off2, size1, size2,
                                                                du, M, kx, ky, dd, opts);
    }
  } else {
    if (opts.nspread == NS) {
...
*/
{
  spread_subproblem_2d_dispatch<T, ::finufft::common::MAX_NSPREAD>(off1, off2, size1, size2, du, M, kx, ky,
                                                dd, opts);
}
...
*/
{
  spread_subproblem_3d_dispatch<T, ::finufft::common::MAX_NSPREAD>(off1, off2, off3, size1, size2, size3, du,
                                                M, kx, ky, kz, dd, opts);
}
...
#pragma omp parallel num_threads(nthr)
  {
    ...
    auto *FINUFFT_RESTRICT ker3 = kernel_values.data() + 2 * ::finufft::common::MAX_NSPREAD;
...
static int interpSorted(
    ...
    T *FINUFFT_RESTRICT kx, T *FINUFFT_RESTRICT ky, T *FINUFFT_RESTRICT kz,
    T *FINUFFT_RESTRICT data_nonuniform, const finufft_spread_opts &opts) {
  return interpSorted_dispatch<T, ::finufft::common::MAX_NSPREAD>(sort_indices, N1, N2, N3, data_uniform, M,
                                               kx, ky, kz, data_nonuniform, opts);
}
...
*/
{
  ...
  if (upsampfac == 2.0)                    // standard sigma (see SISC paper)
    ...
    ns = std::ceil(-log(eps) / (T(::finufft::common::PI) * sqrt(1.0 - 1.0 / upsampfac))); // formula, gam=1
  ...
  if (upsampfac != 2.0) { // again, override beta for custom sigma
    ...
    betaoverns = gamma * T(::finufft::common::PI) * (1.0 - 1.0 / (2 * upsampfac)); // formula based on
                                                                // cutoff
  }
...
T evaluate_kernel_horner_dispatch(T x, const finufft_spread_opts &opts) {
  ...
  */
  ...
  if constexpr (ns == ::finufft::common::MIN_NSPREAD) { // Base case
    if (opts.upsampfac == 2.0) {
      return evaluate_kernel_horner_kernel<T, ::finufft::common::MIN_NSPREAD, 200>(x, opts);
    } else if (opts.upsampfac == 1.25) {
      return evaluate_kernel_horner_kernel<T, ::finufft::common::MIN_NSPREAD, 125>(x, opts);
    } else {
